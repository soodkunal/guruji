...
function g8_video_interactive_playback() {
    ...
    cvlc --play-and-exit "$G8_OUTPUT_LECTURE_FILE" >/dev/null 2>&1 &
    VLC_PID=$!
...
with sr.Microphone() as source:
    try:
        print(" >> Speak now... (say stop word to interrupt)")
...
EOF
    ...
    # 5. Get updated response from OpenAI API
    NEW_GPT_TXT="${G8_OPS_GPT_TXT}.tmp"
    echo " + Getting response using existing transcript..."
    new_text=$(python3 - <<EOF
...
EOF
    ...
    echo "$new_text" > "$NEW_GPT_TXT"

    # 6. Generate new audio from GPT response
    echo " + Generating new audio and video..."
    G8_TMP_TXT="$NEW_GPT_TXT"
    g8_step3_conv_txt_to_wav "$G8_TMP_TXT"

    # 7. Generate updated video
    NEW_OUTPUT_FILE="${G8_OUTPUT_LECTURE_FILE}.tmp"
    g8_run_prog "$NEW_OUTPUT_FILE"

    if [ ! -f "$NEW_OUTPUT_FILE" ]; then
        echo "Updated video was not created. Exiting."
        return 1
    fi

    # 8. Playback updated video
    echo " + Playing updated video..."
    cvlc --play-and-exit "$NEW_OUTPUT_FILE" >/dev/null 2>&1

    # 9. Resume original lecture
    echo " + Resuming original lecture..."
    cvlc "$G8_ORIG_VIDEO" >/dev/null 2>&1
}
